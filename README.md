...
| V1c | Третья ручка: масштаб весов KC→MBON | **предрегистрирована** 9 сентября 2026, спецификация v0.17, хэш `636c49968a0866bd`; не прогнана |
...
### Ступень V1c предзарегистрирована и не прогнана
...
V1b′ — 285 точек, около 3,5 часа на восьми шардах. Верхняя граница выведена из
условия «одиночный спайк одной клетки Кеньона не доводит ни один эталонный MBON
до порога», а не из наблюдения за откликом. **Ожидаемый исход объявлен до
прогона: FAIL-CAL-MBON-CEIL** — категорией, без указания положения полосы.
...
`results/v1c/detector_table.json` (таблица детекторов, некритериальный выход).
