...
**Статус: стенд не построен.** Ни одного прогона с пластичностью не выполнено.
Этот репозиторий открывается до первого содержательного результата намеренно
(whitepaper, раздел 6.4). Пройдены V0 и V1a-S; V1a-D не пройдена и остановлена;
